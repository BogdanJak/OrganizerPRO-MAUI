...
public sealed class HubClient : IAsyncDisposable
{
    ...
    public async Task<List<OrganizerPRO.Application.Common.Interfaces.Identity.UserContext>> GetOnlineUsersAsync(CancellationToken cancellationToken = default)
    {
        var users = await _hubConnection.InvokeAsync<List<OrganizerPRO.Application.Common.Interfaces.Identity.UserContext>>(nameof(ISignalRHub.GetOnlineUsers), cancellationToken).ConfigureAwait(false);
        // Dodaj dyrektywę using dla Microsoft.AspNetCore.SignalR.Client na górze pliku
        return users ?? new List<OrganizerPRO.Application.Common.Interfaces.Identity.UserContext>();
    }
}
